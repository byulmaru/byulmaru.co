...
- raw pixel 비교: `CI=true pnpm run verify:pixels`
- Cloudflare 프리뷰: `CI=true pnpm run preview`

두 검증 명령은 각각 `scripts/verification/react-browser.mjs`와
`scripts/verification/react-pixel-diff.mjs`를 실행하므로 checkout에서 다시 실행할 수 있다. Chrome
실행 파일이 기본 macOS 경로에 없으면 `CHROME_PATH`로 지정한다.
...
`verify:pixels`는 기준선과 React PNG가 각각 정확히 22개이고 파일명·크기가 모두 일치해야 하며,
다음 임계값을 넘으면 종료 코드 1로 실패한다.

- Home/About Us: MAE/RMSE/30 초과 pixel 비율 모두 `0`
- Our Work Overview: MAE `0.2`, RMSE `3`, 30 초과 pixel `0.15%` 이하
- Our Work 나머지 탭: MAE `0.75`, RMSE `4.25`, 30 초과 pixel `0.15%` 이하

최종 실행의 threshold failure는 0개였다.

- Home/About Us 7장은 대응 기준선과 모든 픽셀이 동일했다(MAE/RMSE 0).
- Our Work 15장의 MAE 범위는 `0.0234~0.6620`, RMSE 최대는 `3.9344`였다.
- 채널 차이가 30을 넘는 pixel 비율은 `0~0.1248%`였다.
